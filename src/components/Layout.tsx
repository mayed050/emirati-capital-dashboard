        ...
        >
          ...
          <div className="mt-6 rounded-lg border border-[color:var(--line)] bg-[color:var(--surface-strong)] p-4 text-sm leading-7">
            <div className="flex items-center justify-between gap-2">
              <span className="font-black text-[color:var(--foreground)]">حالة البث اللحظي</span>
              <div className="flex items-center gap-1.5">
                <span className={`inline-block h-2 w-2 rounded-full ${isMarketOpen || forceSim ? "bg-emerald-500 animate-pulse" : "bg-rose-500"}`} />
                <span className="text-xs font-black">{isMarketOpen ? "مفتوح" : forceSim ? "محاكاة نشطة" : "مغلق"}</span>
              </div>
            </div>
            <button
            ...
            >
              {forceSim ? "إيقاف محاكاة 24/7" : "تفعيل محاكاة 24/7"}
            </button>
          </div>
          ...
          <div className="liability-box mt-6 rounded-lg p-4 text-sm leading-7">
            ...
            <p>
              ...
              دمج تجربة مشروع uae-stocks في واجهة منصة الأسهم الإماراتية.
            </p>
          </div>
...
function BrandText() {
  return (
    <span className="min-w-0 w-44 max-w-[calc(100vw-8rem)] text-right md:w-auto md:max-w-none">
      ...
      <span className="block truncate text-xs font-bold text-[color:var(--muted)]">سوق دبي وأبوظبي بقراءة عربية</span>
    </span>
  );
}
